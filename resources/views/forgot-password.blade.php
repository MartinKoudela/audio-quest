...
<head>
    ...
    <title>AudioQuest — Forgot password</title>
...
<nav class="flex items-center justify-between px-8 py-5 border-b border-white/10 backdrop-blur-md">
    ...
    <a href="/login" class="text-gray-400 text-sm hover:text-white transition-colors">Back to login</a>
</nav>
...
<main class="flex-1 flex items-center justify-center px-4 py-16">
    <div class="relative backdrop-blur-2xl bg-white/10 border border-white/20 rounded-[2.5rem] p-10 w-full max-w-sm shadow-2xl text-center">
        ...
        <h1 class="text-2xl font-semibold text-white tracking-tight mb-2">Forgot password?</h1>
        <p class="text-gray-300 text-sm mb-8">Enter your email and we'll send you a reset link.</p>

        @if (session('success'))
            <div class="mb-6 px-4 py-3 rounded-2xl bg-teal-500/20 border border-teal-500/50 text-teal-200 text-sm">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="mt-6 text-xs text-gray-400">
            <a href="/login" class="hover:text-white transition-colors">Back to login</a>
        </div>
